  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-1">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <Badge variant="outline" className="text-[10px] font-bold py-0 h-4 rounded-full border-slate-200 text-slate-400 uppercase">{t("linked")}</Badge>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex flex-col items-end mr-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">{t("status")}</span>
            <Badge
            ...
            >
              {subcontractor.Status ? t(subcontractor.Status.toLowerCase()) : "—"}
            </Badge>
          </div>
          <div className="h-10 w-px bg-slate-200 mx-2 hidden sm:block" />
          <div className="flex flex-col items-end">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">{t("experienceScore")}</span>
      ...
      <div className="space-y-6 max-w-5xl mx-auto">
        {/* Information Section */}
        <div className="space-y-6">
          <SectionCard icon={Building2} iconBg="bg-blue-100" iconColor="text-blue-600" title={t("orgDetails")}>
            <div className="grid gap-5 md:grid-cols-2">
              <div>
                <FieldLabel icon={Building2}>{t("orgName")}</FieldLabel>
                <ReadonlyField value={orgText} />
              </div>
              <div>
                <FieldLabel icon={Globe}>{t("website")}</FieldLabel>
            ...
            <div>
              <FieldLabel icon={MapPin}>{t("address")}</FieldLabel>
              <ReadonlyField value={subcontractor.Address} />
            </div>
          </SectionCard>

          <SectionCard icon={Mail} iconBg="bg-indigo-100" iconColor="text-indigo-600" title={t("contactInfo")}>
            <div className="grid gap-5 md:grid-cols-2">
              <div>
                <FieldLabel icon={Mail}>{t("emails")}</FieldLabel>
                {emails.length > 0 ? (
              ...
              <div>
                <FieldLabel icon={Phone}>{t("phoneNumber")}</FieldLabel>
                <ReadonlyField value={subcontractor.Phone_Number} />
              </div>
            </div>
          </SectionCard>

          <SectionCard icon={CheckCircle} iconBg="bg-emerald-100" iconColor="text-emerald-600" title={t("certsCompliance")}>
            <div className="grid gap-5 md:grid-cols-2">
              <div>
        ...
        <div className="space-y-6">
          <SectionCard icon={Calculator} iconBg="bg-violet-100" iconColor="text-violet-600" title={t("financialSummary")}>
            <div className="grid gap-4 md:grid-cols-2">
              <div className="p-4 rounded-2xl bg-violet-50/50 border border-violet-100 relative overflow-hidden group">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-violet-400">{t("formula")}</span>
              ...
              <div className="p-4 rounded-2xl bg-emerald-50/50 border border-emerald-100 relative overflow-hidden group">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-400">{t("adjFormula")}</span>
                ...
                <p className="text-[10px] text-emerald-400 font-medium whitespace-nowrap">{t("costsPlusApprovedCos")}</p>
              </div>
            </div>
          </SectionCard>

          <SectionCard icon={Package} iconBg="bg-slate-100" iconColor="text-slate-600" title={t("purchaseOrders")}>
            <div className="space-y-4">
              {loadingOrders ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-slate-300 gap-2">
                  ...
                  <p className="text-sm font-semibold uppercase tracking-widest opacity-50">{t("noOrdersAssigned")}</p>
                </div>
              ) : (
                orders.map((order) => (
                  <div key={order.ID_Order} className="group rounded-2xl border border-slate-200 bg-white shadow-sm hover:border-gqm-green/30 hover:shadow-md transition-all">
                    <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/30 group-hover:bg-white transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="font-bold text-slate-700 leading-none">{order.Title || t("standardOrder")}</h4>
                    ...
                    <div className="p-4 space-y-4">
                      <div className="grid grid-cols-3 gap-2 py-1">
                        <div className="flex flex-col">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-2">{t("formula")}</span>
                          <span className="font-bold text-slate-600 tabular-nums text-base">${(order.Formula || 0).toFixed(2)}</span>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-2">{t("adjFormula")}</span>
                          <span className="font-black text-slate-800 tabular-nums text-base">${(order.Adj_formula || 0).toFixed(2)}</span>
                        </div>
                        <div className="flex flex-col items-end">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-2">{t("items")}</span>
                      ...
                      {order.change_orders?.length > 0 && (
                        <div className="pt-3 border-t border-slate-100">
                          <div className="flex items-center gap-2 mb-2.5">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t("changeOrders")}</span>
                          </div>
                      ...
                      >
                        <Plus className="h-3.5 w-3.5 mr-2" /> {t("addChangeOrder")}
                      </Button>
                    </div>
                  </div>
                ))
