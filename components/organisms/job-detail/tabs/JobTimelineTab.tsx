...
const TL_PERIODS: TLPeriod[] = ["day", "week", "month"]
...
function PDFPopover({ jobId, onClose }: { jobId: string; onClose: () => void }) {
  const t = useTranslations("jobTimelineTab.pdf")
  const te = useTranslations("jobTimelineTab.error")
  ...
  async function handleDownload() {
    ...
    try {
      ...
      setDlError(e?.message ?? te("pdfFail"))
    } finally {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: "6px", marginBottom: "12px" }}>
        {TL_PERIODS.map(p => (
          <button key={p} onClick={() => setPeriod(p)} style={{ flex: 1, padding: "5px 0", fontSize: "11px", fontWeight: period === p ? 600 : 400, borderRadius: "6px", border: `1.5px solid ${period === p ? "#0B2E1E" : "#D1D5DB"}`, background: period === p ? "#0B2E1E" : "transparent", color: period === p ? "#fff" : "#374151", cursor: "pointer", transition: "all 0.12s ease" }}>
            {t(`periods.${p}`)}
          </button>
        ))}
      </div>
      <div style={{ marginBottom: "4px" }}>
        ...
        <p style={{ fontSize: "10px", color: "#9CA3AF", margin: "4px 0 12px" }}>
          {t("refDateHint", { period: t(`periods.${period}`).toLowerCase() })}
        </p>
      </div>
      {dlError && (
      ...
      <button onClick={handleDownload} disabled={downloading} style={{ width: "100%", padding: "9px", background: downloading ? "#9CA3AF" : "linear-gradient(135deg, #0B2E1E, #1A5C3A)", color: "#fff", border: "none", borderRadius: "8px", fontSize: "13px", fontWeight: 600, cursor: downloading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", transition: "all 0.15s ease" }}>
        {downloading ? (
          <><span style={{ width: "12px", height: "12px", border: "2px solid rgba(255,255,255,0.3)", borderTopColor: "#fff", borderRadius: "50%", display: "inline-block", animation: "tl-spin 0.7s linear infinite" }} />{t("btnGenerating")}</>
        ) : <>{t("btnDownload")}</>}
      </button>
    </div>
  )
      ...
      <div className="px-6 py-6">
        ...
        {!tl.loading && !tl.error && tl.entries.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
            ...
            <p className="text-sm font-medium text-slate-500">{t("empty.title")}</p>
            <p className="text-xs text-slate-400">{t("empty.desc")}</p>
          </div>
        )}
