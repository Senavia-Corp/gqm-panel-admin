...
export function JobCommissionsTab({ job, reload }: JobCommissionsTabProps) {
  const t = useTranslations("jobCommissionsTab")
  const tm = useTranslations("jobCommissionsTab.months")
  ...
  const updateType = async (detailId: string, newType: string) => {
    try {
      ...
      if (!res.ok) throw new Error(t("toasts.errorFailUpdate"))
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-0.5">{t("card.labelMember")}</p>
                      <p className="text-sm font-bold text-slate-900">{member?.Member_Name ?? t("card.unknownMember")}</p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-0.5">{t("card.labelRole")}</p>
                ...
                <div className="space-y-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-0.5">{t("card.labelPeriod")}</p>
                      <p className="text-sm font-semibold text-slate-700">
                        {commission?.Month ? tm(commission.Month.toLowerCase()) : "???"} {commission?.Year ?? "????"}
                      </p>
                    </div>
                  </div>

                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">{t("card.labelType")}</p>
                ...
                <div className="flex flex-col gap-3 rounded-2xl bg-slate-50 p-5 border border-slate-100 shadow-inner">
                  <div className="flex items-center justify-between border-b border-slate-200/50 pb-2">
                    <div className="flex items-center gap-1.5 text-slate-500">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-widest">{t("card.labelFactor")}</span>
                    </div>
                  ...
                  <div className="flex items-center justify-between border-b border-slate-200/50 pb-2">
                    <div className="flex items-center gap-1.5 text-slate-500">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-widest">{t("card.labelPremium")}</span>
                    </div>
                  ...
                  <div className="flex flex-col gap-1 pt-1">
                    <div className="flex items-center gap-1.5 text-emerald-700">
                      <DollarSign className="h-3.5 w-3.5" />
                      <span className="text-[10px] font-bold uppercase tracking-widest">{t("card.labelValue")}</span>
                    </div>
                  ...
                  <div className="mt-auto pt-2 border-t border-slate-200/50">
                    <p className="text-[10px] text-slate-400 text-right uppercase font-bold tracking-tighter">{t("card.verified")}</p>
                  </div>
                </div>
              </div>
            </CardContent>
