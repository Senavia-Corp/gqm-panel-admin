    ...
    <div className="rounded-lg border bg-white">
      ...
      <div className="md:hidden divide-y divide-slate-100">
        {jobs.map((job) => {
          ...
          return (
            <div key={job.ID_Jobs} className="p-4 flex flex-col gap-3">
              {/* Job ID + Status */}
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-[10px] font-mono font-semibold uppercase tracking-widest text-slate-400">{t("colJobId")}</p>
                  <p className="mt-0.5 text-sm font-bold text-slate-900">{job.ID_Jobs}</p>
                </div>
              ...
              {repName && (
                <div className="flex items-center gap-2.5">
                  <RepAvatar />
                  <p className="text-sm text-slate-700">{repName}</p>
                </div>
              )}
