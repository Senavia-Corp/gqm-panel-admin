...
export function ParentCompanyTimelineTab({ pmcId }: { pmcId: string }) {
  ...
  const fetchPage = useCallback(async (page: number, append: boolean) => {
    ...
    try {
      const res  = await apiFetch(`/api/timeline/parent-mgmt-co/${encodeURIComponent(pmcId)}?page=${page}&limit=${PAGE_LIMIT}`)
      ...
      <div className="px-6 py-6">
        ...
        {!tl.loading && !tl.error && tl.entries.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
            ...
            <p className="text-xs text-slate-400">Events will appear here as changes happen on this parent company.</p>
          </div>
        )}
