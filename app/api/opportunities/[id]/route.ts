...
export async function GET(request: NextRequest, { params }: Params) {
  ...
  const result = await proxyFetch(`${OPP_BASE}/${id}`, { method: "GET" }, authHeader)
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  const result = await proxyFetch(`${OPP_BASE}/${id}`, { method: "PATCH", body: JSON.stringify(body) }, authHeader)
  if (!result.ok) return jsonError(`API error (${result.status})`, result.status, { detail: result.error })
  return NextResponse.json(result.data)
}
...
export async function DELETE(request: NextRequest, { params }: Params) {
  const { id } = await params
  const authHeader = request.headers.get("Authorization")
  const result = await proxyFetch(`${OPP_BASE}/${id}`, { method: "DELETE" }, authHeader)
  if (!result.ok) return jsonError(`API error (${result.status})`, result.status, { detail: result.error })
  return NextResponse.json(result.data)
}
