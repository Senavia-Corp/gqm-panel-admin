...
const OPP_BASE = `${BASE}/opportunities`
...
/** POST /api/opportunities/[id]/applicants/[subId] — link subcontractor */
export async function POST(request: NextRequest, { params }: Params) {
  const { id, subId } = await params
  const authHeader = request.headers.get("Authorization")
  const url = `${BASE}/opportunities_subcontractors/opportunities/${id}/subcontractors/${subId}`
...
/** PATCH /api/opportunities/[id]/applicants/[subId] — update application state */
export async function PATCH(request: NextRequest, { params }: Params) {
  const { id, subId } = await params
  const authHeader = request.headers.get("Authorization")
  const body = await request.json().catch(() => null)
  if (!body) return jsonError("Invalid JSON body", 400)
  const result = await proxyFetch(`${OPP_BASE}/${id}/applicants/${subId}`, { method: "PATCH", body: JSON.stringify(body) }, authHeader)
  if (!result.ok) return jsonError(`API error (${result.status})`, result.status, { detail: result.error })
  return NextResponse.json(result.data)
}

/** DELETE /api/opportunities/[id]/applicants/[subId] — unlink subcontractor */
