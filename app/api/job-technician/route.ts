// app/api/job-technician/route.ts
import { type NextRequest, NextResponse } from "next/server"
import { getBackendUrl } from "@/lib/api-utils"

const PYTHON_BASE_URL = `${getBackendUrl()}/`
const PYTHON_API_URL  = `${PYTHON_BASE_URL}job_technician`
...
export async function POST(request: NextRequest) {
  ...
  const { jobId, technicianId } = body as any
  if (!jobId || !technicianId) return jsonError("jobId and technicianId are required", 400)

  const url = `${PYTHON_API_URL}/jobs/${encodeURIComponent(jobId)}/technicians/${encodeURIComponent(technicianId)}`
