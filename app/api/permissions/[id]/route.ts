...
export async function GET(_req: Request, { params }: Params) {
  try {
    ...
    const response = await fetch(`${PYTHON_API_BASE_URL}permission/${id}`, {
      method: "GET",
    ...
    console.error("[proxy] permissions/[id] GET error:", error)
...
export async function PATCH(req: Request, { params }: Params) {
  try {
    ...
    console.error("[proxy] permissions/[id] PATCH error:", error)
    return NextResponse.json(
      { detail: "Proxy error calling Python API", code: "proxy_error", error: error?.message ?? String(error) },
      { status: 500 }
    )
...
export async function DELETE(_req: Request, { params }: Params) {
  try {
    ...
    console.error("[proxy] permissions/[id] DELETE error:", error)
    return NextResponse.json(
      { detail: "Proxy error calling Python API", code: "proxy_error", error: error?.message ?? String(error) },
      { status: 500 }
    )
