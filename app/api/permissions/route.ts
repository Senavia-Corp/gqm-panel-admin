...
export async function GET(req: Request) {
  try {
    ...
    const target = `${PYTHON_API_BASE_URL}permission/${qs ? `?${qs}` : ""}`
    ...
    console.error("[proxy] permissions GET error:", error)
...
export async function POST(req: Request) {
  try {
    ...
    const response = await fetch(`${PYTHON_API_BASE_URL}permission/`, {
      method: "POST",
      headers: { "Content-Type": "application/json", ...(authHeader ? { Authorization: authHeader } : {}) },
    ...
    console.error("[proxy] permissions POST error:", error)
    return NextResponse.json(
      { detail: "Proxy error calling Python API", code: "proxy_error", error: error?.message ?? String(error) },
      { status: 500 }
    )
