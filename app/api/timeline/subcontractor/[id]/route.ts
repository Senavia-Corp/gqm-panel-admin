// app/api/timeline/subcontractor/[id]/route.ts
...
export async function GET(
  ...
  const idSubc = String(id ?? "").trim()
  if (!idSubc) return jsonError("Missing subcontractor id", 400)
  ...
  const url = `${PYTHON_BASE_URL.replace(/\/$/, "")}/tlactivity/subcontractor/${encodeURIComponent(idSubc)}${qs ? `?${qs}` : ""}`
  console.log("[timeline proxy] GET by ID_Subcontractor ->", url)
