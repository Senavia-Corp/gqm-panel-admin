// app/api/timeline/parent-mgmt-co/[id]/route.ts
...
export async function GET(
  ...
  const idPmc = String(id ?? "").trim()
  if (!idPmc) return jsonError("Missing parent mgmt co id", 400)
  ...
  const url = `${PYTHON_BASE_URL.replace(/\/$/, "")}/tlactivity/parent-mgmt-co/${encodeURIComponent(idPmc)}${qs ? `?${qs}` : ""}`
  console.log("[timeline proxy] GET by ID_Community_Tracking ->", url)
