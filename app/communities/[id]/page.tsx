...
const SKIP_ON_PATCH: Array<keyof Client> = ["jobs", "manager", "members", "ID_Client"]
...
function StatusBadge({ status }: { status?: string | null }) {
  ...
  return <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${map[status.toLowerCase()] ?? "bg-blue-100 text-blue-700 border-blue-200"}`}>{status}</span>
}
...
function JobCard({ job, onClick }: { job: Job; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick}
      className="group w-full rounded-xl border border-slate-100 bg-white p-4 text-left shadow-sm transition-all hover:border-violet-200 hover:shadow-md">
      {/* Header */}
      ...
      <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500">
        {job.Service_type && (
          <span className="flex items-center gap-1"><Wrench className="h-3 w-3 text-slate-400" />{job.Service_type}</span>
        )}
        {assignedDate && (
          <span className="flex items-center gap-1"><Calendar className="h-3 w-3 text-slate-400" />{assignedDate}</span>
        )}
        {price && (
        ...
        {(job.Gqm_total_change_orders ?? 0) > 0 && (
          <span className="flex items-center gap-1 text-orange-600">
            <Tag className="h-3 w-3" />{job.Gqm_total_change_orders} CO{job.Gqm_total_change_orders !== 1 ? "s" : ""}
          </span>
        )}
        ...
        return (
          <button key={id} onClick={() => onChange(id)}
            className={`relative flex items-center gap-2 whitespace-nowrap px-5 py-3 text-sm font-medium transition-colors
              ${on ? "text-emerald-700 after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-emerald-600"
...
export default function ClientDetailPage({ params }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const n: Client = { ...upd, jobs: client?.jobs ?? [], manager: client?.manager ?? [], members: client?.members ?? [],
        Email_Address: parseArrayField(upd.Email_Address), Phone_Number: parseArrayField(upd.Phone_Number) }
  ...
  const eEmails = useMemo(() => parseArrayField(formData.Email_Address), [formData.Email_Address])
  const ePhones = useMemo(() => parseArrayField(formData.Phone_Number), [formData.Phone_Number])
      ...
      <div className="mx-auto max-w-xl p-6">
        <Card className="p-6">
          <div className="mb-3 flex items-center gap-2 text-red-500">
            ...
            <h1 className="text-lg font-semibold">Community could not be loaded</h1>
          </div>
                  ...
                  : <>
                      ...
                      {editedFields.size > 0 && (
                        <Button size="sm" disabled={saving} onClick={handleSave}
                          className="h-8 bg-gqm-green hover:bg-gqm-green/90 gap-1.5 text-xs">
              ...
              <Card className="p-6">
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  ...
                  <div className="md:col-span-2">
                    ...
                    {isEditing
                      ...
                      : <div className="flex items-start gap-1.5 text-sm text-slate-700">
                          ...
                          <span>{formData.Address ?? <span className="italic text-slate-400">No address</span>}</span>
                        </div>}
                  </div>
                  <div>
                    <Label className="mb-1.5 block text-sm font-medium">Status</Label>
                    {isEditing
                      ? <Input value={formData.Client_Status ?? ""} onChange={(e) => set_("Client_Status", e.target.value)} className={ch("Client_Status")} placeholder="active / inactive…" />
                  ...
                  <div>
                    ...
                    <Input disabled={!isEditing} value={formData.Compliance_Partner ?? ""}
                      onChange={(e) => set_("Compliance_Partner", e.target.value)} className={ch("Compliance_Partner")} />
                  </div>
                  <div>
                    <Label className="mb-1.5 block text-sm font-medium">Risk Value</Label>
                    <Input disabled={!isEditing} value={formData.Risk_Value ?? ""}
                      onChange={(e) => set_("Risk_Value", e.target.value)} className={ch("Risk_Value")} />
                  </div>
                  <div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <Textarea disabled={!isEditing} value={formData.Services_interested_in ?? ""}
                      onChange={(e) => set_("Services_interested_in", e.target.value)} className={ch("Services_interested_in")} rows={2} />
                  </div>
                  <div className="md:col-span-2">
            ...
            <div className="space-y-4">
              <Card className="p-5">
                ...
                <dl className="space-y-3">
                  {[{ label: "Client ID", value: client.ID_Client }, { label: "Parent Co.", value: client.parent_mgmt_co.Property_mgmt_co }, { label: "Podio ID", value: client.podio_item_id }].map(({ label, value }) => (
                    <div key={label}>
                ...
                {client.manager!.map((mgr) => (
                  <div key={mgr.ID_Manager} className="flex items-start gap-3 rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {mgr.Manager_email && (
                        <a href={`mailto:${mgr.Manager_email}`} className="block truncate text-xs text-slate-500 hover:text-emerald-600">{mgr.Manager_email}</a>
                      )}
                      {mgr.Manager_location && (
                        <p className="flex items-center gap-1 truncate text-xs text-slate-400 mt-0.5"><MapPin className="h-3 w-3" />{mgr.Manager_location}</p>
                      )}
                    ...
                    <button onClick={() => unlinkMgr(mgr.ID_Manager)} disabled={unlinkingManager === mgr.ID_Manager}
                      className="mt-0.5 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-md text-slate-300 hover:bg-red-50 hover:text-red-500 disabled:opacity-50 transition-colors" title="Unlink">
                ...
                {client.members!.map((mem) => (
                  <div key={mem.ID_Member} className="flex items-start gap-3 rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {mem.Email_Address && (
                        <a href={`mailto:${mem.Email_Address}`} className="block truncate text-xs text-slate-400 hover:text-blue-600 mt-0.5">{mem.Email_Address}</a>
                      )}
