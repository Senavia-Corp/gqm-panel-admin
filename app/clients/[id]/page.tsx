...
export default function ParentMgmtCoDetailsPage({ params }: ParentMgmtCoDetailsPageProps) {
  ...
  const parentMgmtCoId = resolvedParams?.id
  ...
  const fetchParentMgmtCoData = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch parent mgmt co (${response.status})`)
      }
      ...
      toast({
        title: "Error",
        description: "Failed to load parent management company data",
        variant: "destructive"
      })
    } finally {
      setLoading(false)
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to update parent mgmt co")
      }

      const updated = (await response.json()) as ParentMgmtCo
      const normalized: ParentMgmtCo = {
        ...updated,
        clients: Array.isArray(updated.clients) ? updated.clients : []
      }
      ...
      toast({
        title: "Success",
        description: "Parent management company updated successfully"
      })
    } catch (error) {
      console.error("Error updating parent mgmt co:", error)
      toast({
        title: "Error",
        description: "Failed to update parent management company",
        variant: "destructive"
      })
    }
  }
          ...
          <main className="flex-1 overflow-y-auto p-6">
            <div className="mx-auto max-w-xl">
              <Card className="p-6">
                ...
                {parentMgmtCoId ? <p className="mt-4 text-xs text-muted-foreground">ID: {parentMgmtCoId}</p> : null}
              </Card>
            </div>
          </main>
        </div>
